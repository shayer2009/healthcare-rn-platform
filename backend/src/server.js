...
app.post("/api/admin/login", asyncHandler(async (req, res) => {
  ...
  const userStr = JSON.stringify(JSON.stringify(user));
  res.set("Cache-Control", "no-store");
  res.type("html").send("<!DOCTYPE html><html><head><meta charset=\"utf-8\"><title>Redirecting...</title></head><body style=\"font-family:system-ui;text-align:center;padding:3rem;\"><p>Login successful. Redirecting...</p><script>sessionStorage.setItem(\"admin_token\"," + tokenStr + ");sessionStorage.setItem(\"admin_user\"," + userStr + ");window.location.href=\"/admin#" + hash.replace(/"/g, "&quot;") + "\";</script><noscript><meta http-equiv=\"refresh\" content=\"0;url=/admin#" + hash.replace(/"/g, "&quot;") + "\"></noscript></body></html>");
}));
...
"              throw new Error(errObj.message || \"HTTP \" + r.status);" +
"            } catch(parseErr) {" +
...
"    appEl.innerHTML = \"<div class=\\\"err\\\"><p>Error: \" + errMsg.replace(/</g, \"&lt;\").replace(/>/g, \"&gt;\") + \"</p><p><a href=\\\"/api/admin/login\\\">Back to Login</a></p></div>\";" +
"  }" +
